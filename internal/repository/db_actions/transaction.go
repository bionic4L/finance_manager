...
func (t TransactionRepository) Transaction(ctx context.Context, fromID, toID, amount int) error {
	...
	if err := t.DBMinusMoneyUserFrom(fromID, amount); err != nil {
		return err
	}
...
func (d *TransactionRepository) DBMinusMoneyUserFrom(fromID, amount int) error {
	_, err := d.db.Exec(queryUpdateUserBalanceMinus, amount, fromID)
	if err != nil {
		return err
	}
